...
static int seed_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
    while(inl>=((size_t)1<<(sizeof(long)*8-2)))
    {
        SEED_cbc_encrypt(in, out, (long)((size_t)1<<(sizeof(long)*8-2)), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_encrypting(ctx));
        inl-=((size_t)1<<(sizeof(long)*8-2));
        in +=((size_t)1<<(sizeof(long)*8-2));
        out+=((size_t)1<<(sizeof(long)*8-2));
    }
    if (inl) SEED_cbc_encrypt(in, out, (long)inl, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_encrypting(ctx));
    return 1;
}
static int seed_cfb128_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
    ...
    while (inl && inl >= chunk)
    {
        ...
        SEED_cfb128_encrypt(in, out, (long) ((128 == 1) && !EVP_CIPHER_CTX_test_flags(ctx, 0x2000) ? inl*8 : inl), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_iv_noconst(ctx), &num, EVP_CIPHER_CTX_encrypting(ctx));
...
static int seed_ecb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
    size_t i, bl;
    bl = EVP_CIPHER_CTX_cipher(ctx)->block_size;
    if (inl < bl) return 1;
    inl -= bl;
    for (i=0; i <= inl; i+=bl) SEED_ecb_encrypt(in + i, out + i, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_encrypting(ctx));
    return 1;
}
static int seed_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
    while(inl>=((size_t)1<<(sizeof(long)*8-2)))
    {
        int num = EVP_CIPHER_CTX_num(ctx);
        SEED_ofb128_encrypt(in, out, (long)((size_t)1<<(sizeof(long)*8-2)), &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_iv_noconst(ctx), &num);
        EVP_CIPHER_CTX_set_num(ctx, num);
        inl-=((size_t)1<<(sizeof(long)*8-2));
        in +=((size_t)1<<(sizeof(long)*8-2));
        out+=((size_t)1<<(sizeof(long)*8-2));
    ...
    {
        int num = EVP_CIPHER_CTX_num(ctx);
        SEED_ofb128_encrypt(in, out, (long)inl, &((EVP_SEED_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx))->ks, EVP_CIPHER_CTX_iv_noconst(ctx), &num);
        EVP_CIPHER_CTX_set_num(ctx, num);
    }
    return 1;
}
static const EVP_CIPHER seed_cbc = { 777, 16, 16, 16, 0x1000 | 0x2, seed_init_key, seed_cbc_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, ((void *)0) };
const EVP_CIPHER *EVP_seed_cbc(void)
{
    return &seed_cbc;
}
static const EVP_CIPHER seed_cfb128 = { 779, 1, 16, 16, 0x1000 | 0x3, seed_init_key, seed_cfb128_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, ((void *)0) };
const EVP_CIPHER *EVP_seed_cfb128(void)
{
    return &seed_cfb128;
}
static const EVP_CIPHER seed_ofb = { 778, 1, 16, 16, 0x1000 | 0x4, seed_init_key, seed_ofb_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, ((void *)0) };
const EVP_CIPHER *EVP_seed_ofb(void)
{
    return &seed_ofb;
}
static const EVP_CIPHER seed_ecb = { 776, 16, 16, 0, 0x1000 | 0x1, seed_init_key, seed_ecb_cipher, 0, sizeof(EVP_SEED_KEY), 0, 0, 0, ((void *)0) };
const EVP_CIPHER *EVP_seed_ecb(void)
{
    return &seed_ecb;
}
