...
#line 20 "crypto\\des\\set_key.c"
int *_shadow_DES_check_key(void) { static int _hide_DES_check_key=0; return &_hide_DES_check_key; }
...
in = &(*key)[0];
(c =((DES_LONG)(*((in)++))) , c|=((DES_LONG)(*((in)++)))<< 8L, c|=((DES_LONG)(*((in)++)))<<16L, c|=((DES_LONG)(*((in)++)))<<24L);
(d =((DES_LONG)(*((in)++))) , d|=((DES_LONG)(*((in)++)))<< 8L, d|=((DES_LONG)(*((in)++)))<<16L, d|=((DES_LONG)(*((in)++)))<<24L);
...
*(k++) = (_lrotr(t2,30)) & 0xffffffffL;
t2 = ((s >> 16L) | (t & 0xffff0000L));
*(k++) = (_lrotr(t2,26)) & 0xffffffffL;
}
}
int DES_key_sched(const_DES_cblock *key, DES_key_schedule *schedule)
{
