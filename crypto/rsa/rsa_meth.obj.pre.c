...
#line 13 "crypto\\rsa\\rsa_meth.c"
RSA_METHOD *RSA_meth_new(const char *name, int flags)
{
RSA_METHOD *meth = CRYPTO_zalloc(sizeof(*meth), "crypto\\rsa\\rsa_meth.c", 16);
if (meth != ((void *)0)) {
meth->flags = flags;
meth->name = CRYPTO_strdup(name, "crypto\\rsa\\rsa_meth.c", 21);
if (meth->name != ((void *)0))
return meth;
CRYPTO_free(meth, "crypto\\rsa\\rsa_meth.c", 25);
}
ERR_put_error(4,(162),((1|64)),"crypto\\rsa\\rsa_meth.c",28);
return ((void *)0);
}
void RSA_meth_free(RSA_METHOD *meth)
{
if (meth != ((void *)0)) {
CRYPTO_free(meth->name, "crypto\\rsa\\rsa_meth.c", 35);
CRYPTO_free(meth, "crypto\\rsa\\rsa_meth.c", 36);
}
}
RSA_METHOD *RSA_meth_dup(const RSA_METHOD *meth)
{
RSA_METHOD *ret = CRYPTO_malloc(sizeof(*ret), "crypto\\rsa\\rsa_meth.c", 42);
if (ret != ((void *)0)) {
memcpy(ret, meth, sizeof(*meth));
ret->name = CRYPTO_strdup(meth->name, "crypto\\rsa\\rsa_meth.c", 47);
if (ret->name != ((void *)0))
return ret;
CRYPTO_free(ret, "crypto\\rsa\\rsa_meth.c", 51);
}
ERR_put_error(4,(161),((1|64)),"crypto\\rsa\\rsa_meth.c",54);
return ((void *)0);
}
const char *RSA_meth_get0_name(const RSA_METHOD *meth)
{
...
int RSA_meth_set1_name(RSA_METHOD *meth, const char *name)
{
char *tmpname = CRYPTO_strdup(name, "crypto\\rsa\\rsa_meth.c", 65);
if (tmpname == ((void *)0)) {
ERR_put_error(4,(163),((1|64)),"crypto\\rsa\\rsa_meth.c",68);
return 0;
}
CRYPTO_free(meth->name, "crypto\\rsa\\rsa_meth.c", 72);
meth->name = tmpname;
return 1;
}
int RSA_meth_get_flags(RSA_METHOD *meth)
