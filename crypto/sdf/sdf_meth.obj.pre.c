...
#line 52 "crypto\\sdf\\sdf_meth.c"
#line 1 "D:\\07.OpenSrc\\GmSSL\\include\\openssl/crypto.h"
#line 53 "crypto\\sdf\\sdf_meth.c"
...
#line 54 "crypto\\sdf\\sdf_meth.c"
#line 1 "D:\\07.OpenSrc\\GmSSL\\include\\internal/sdf_int.h"
#line 1 "D:\\07.OpenSrc\\GmSSL\\include\\openssl/gmsdf.h"
#line 55 "D:\\07.OpenSrc\\GmSSL\\include\\internal/sdf_int.h"
#line 1 "D:\\07.OpenSrc\\GmSSL\\include\\internal/dso.h"
#line 56 "D:\\07.OpenSrc\\GmSSL\\include\\internal/sdf_int.h"
...
#line 468 "D:\\07.OpenSrc\\GmSSL\\include\\internal/sdf_int.h"
#line 55 "crypto\\sdf\\sdf_meth.c"
SDF_METHOD *SDF_METHOD_load_library(const char *so_path)
{
    SDF_METHOD *ret = ((void *)0);
    SDF_METHOD *sdf = ((void *)0);
    if (!(sdf = CRYPTO_zalloc(sizeof(*sdf), "crypto\\sdf\\sdf_meth.c", 67)))
    {
        ERR_put_error(63,(141),((1|64)),"crypto\\sdf\\sdf_meth.c",68);
        goto end;
    }
    if (!(sdf->dso = DSO_load(((void *)0), so_path, ((void *)0), 0)))
    {
        ERR_put_error(63,(141),(110),"crypto\\sdf\\sdf_meth.c",72);
        goto end;
    }
    sdf->OpenDevice = (SDF_OpenDevice_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""OpenDevice");
    sdf->CloseDevice = (SDF_CloseDevice_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""CloseDevice");
    sdf->OpenSession = (SDF_OpenSession_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""OpenSession");
    sdf->CloseSession = (SDF_CloseSession_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""CloseSession");
    sdf->GetDeviceInfo = (SDF_GetDeviceInfo_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GetDeviceInfo");
    sdf->GenerateRandom = (SDF_GenerateRandom_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateRandom");
    sdf->GetPrivateKeyAccessRight = (SDF_GetPrivateKeyAccessRight_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GetPrivateKeyAccessRight");
    sdf->ReleasePrivateKeyAccessRight = (SDF_ReleasePrivateKeyAccessRight_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ReleasePrivateKeyAccessRight");
    sdf->ExportSignPublicKey_RSA = (SDF_ExportSignPublicKey_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExportSignPublicKey_RSA");
    sdf->ExportEncPublicKey_RSA = (SDF_ExportEncPublicKey_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExportEncPublicKey_RSA");
    sdf->GenerateKeyPair_RSA = (SDF_GenerateKeyPair_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyPair_RSA");
    sdf->GenerateKeyWithIPK_RSA = (SDF_GenerateKeyWithIPK_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithIPK_RSA");
    sdf->GenerateKeyWithEPK_RSA = (SDF_GenerateKeyWithEPK_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithEPK_RSA");
    sdf->ImportKeyWithISK_RSA = (SDF_ImportKeyWithISK_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ImportKeyWithISK_RSA");
    sdf->ExchangeDigitEnvelopeBaseOnRSA = (SDF_ExchangeDigitEnvelopeBaseOnRSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExchangeDigitEnvelopeBaseOnRSA");
    sdf->ExportSignPublicKey_ECC = (SDF_ExportSignPublicKey_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExportSignPublicKey_ECC");
    sdf->ExportEncPublicKey_ECC = (SDF_ExportEncPublicKey_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExportEncPublicKey_ECC");
    sdf->GenerateKeyPair_ECC = (SDF_GenerateKeyPair_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyPair_ECC");
    sdf->GenerateKeyWithIPK_ECC = (SDF_GenerateKeyWithIPK_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithIPK_ECC");
    sdf->GenerateKeyWithEPK_ECC = (SDF_GenerateKeyWithEPK_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithEPK_ECC");
    sdf->ImportKeyWithISK_ECC = (SDF_ImportKeyWithISK_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ImportKeyWithISK_ECC");
    sdf->GenerateAgreementDataWithECC = (SDF_GenerateAgreementDataWithECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateAgreementDataWithECC");
    sdf->GenerateKeyWithECC = (SDF_GenerateKeyWithECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithECC");
    sdf->GenerateAgreementDataAndKeyWithECC = (SDF_GenerateAgreementDataAndKeyWithECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateAgreementDataAndKeyWithECC");
    sdf->ExchangeDigitEnvelopeBaseOnECC = (SDF_ExchangeDigitEnvelopeBaseOnECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExchangeDigitEnvelopeBaseOnECC");
    sdf->GenerateKeyWithKEK = (SDF_GenerateKeyWithKEK_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""GenerateKeyWithKEK");
    sdf->ImportKeyWithKEK = (SDF_ImportKeyWithKEK_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ImportKeyWithKEK");
    sdf->DestroyKey = (SDF_DestroyKey_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""DestroyKey");
    sdf->ExternalPublicKeyOperation_RSA = (SDF_ExternalPublicKeyOperation_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExternalPublicKeyOperation_RSA");
    sdf->InternalPrivateKeyOperation_RSA = (SDF_InternalPrivateKeyOperation_RSA_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""InternalPrivateKeyOperation_RSA");
    sdf->ExternalVerify_ECC = (SDF_ExternalVerify_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExternalVerify_ECC");
    sdf->InternalSign_ECC = (SDF_InternalSign_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""InternalSign_ECC");
    sdf->InternalVerify_ECC = (SDF_InternalVerify_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""InternalVerify_ECC");
    sdf->ExternalEncrypt_ECC = (SDF_ExternalEncrypt_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ExternalEncrypt_ECC");
    sdf->InternalEncrypt_ECC = (SDF_InternalEncrypt_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""InternalEncrypt_ECC");
    sdf->InternalDecrypt_ECC = (SDF_InternalDecrypt_ECC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""InternalDecrypt_ECC");
    sdf->Encrypt = (SDF_Encrypt_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""Encrypt");
    sdf->Decrypt = (SDF_Decrypt_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""Decrypt");
    sdf->CalculateMAC = (SDF_CalculateMAC_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""CalculateMAC");
    sdf->HashInit = (SDF_HashInit_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""HashInit");
    sdf->HashUpdate = (SDF_HashUpdate_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""HashUpdate");
    sdf->HashFinal = (SDF_HashFinal_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""HashFinal");
    sdf->CreateObject = (SDF_CreateObject_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""CreateFile");
    sdf->ReadObject = (SDF_ReadObject_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""ReadFile");
    sdf->WriteObject = (SDF_WriteObject_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""WriteFile");
    sdf->DeleteObject = (SDF_DeleteObject_FuncPtr)DSO_bind_func(sdf->dso, "SDF_""DeleteFile");
    ret = sdf;
    sdf = ((void *)0);
end:
    SDF_METHOD_free(sdf);
...
void SDF_METHOD_free(SDF_METHOD *meth)
{
    ...
    CRYPTO_free(meth, "crypto\\sdf\\sdf_meth.c", 136);
}
