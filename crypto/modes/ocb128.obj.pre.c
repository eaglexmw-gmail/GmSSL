...
tmp_ptr =
CRYPTO_realloc(ctx->l, ctx->max_l_index * sizeof(OCB_BLOCK), "crypto\\modes\\ocb128.c", 122);
if (tmp_ptr == ((void *)0)) 
return ((void *)0);
ctx->l = tmp_ptr;
}
...
OCB128_CONTEXT *octx;
int ret;
if ((octx = CRYPTO_malloc(sizeof(*octx), "crypto\\modes\\ocb128.c", 146)) != ((void *)0)) {
ret = CRYPTO_ocb128_init(octx, keyenc, keydec, encrypt, decrypt,
stream);
if (ret)
return octx;
CRYPTO_free(octx, "crypto\\modes\\ocb128.c", 151);
}
return ((void *)0);
}
int CRYPTO_ocb128_init(OCB128_CONTEXT *ctx, void *keyenc, void *keydec,
...
ctx->l_index = 0;
ctx->max_l_index = 5;
ctx->l = CRYPTO_malloc(ctx->max_l_index * 16, "crypto\\modes\\ocb128.c", 167);
if (ctx->l == ((void *)0))
return 0;
ctx->encrypt = encrypt;
ctx->decrypt = decrypt;
...
dest->keydec = keydec;
if (src->l) {
dest->l = CRYPTO_malloc(src->max_l_index * 16, "crypto\\modes\\ocb128.c", 213);
if (dest->l == ((void *)0))
return 0;
...
if (lookup == ((void *)0))
return 0;
( (&ctx->offset_aad)->a[0]=(&ctx->offset_aad)->a[0]^(lookup)->a[0], (&ctx->offset_aad)->a[1]=(&ctx->offset_aad)->a[1]^(lookup)->a[1] );
aad_block = (OCB_BLOCK *)(aad + ((i - ctx->blocks_hashed - 1) * 16));
( (&tmp1)->a[0]=(&ctx->offset_aad)->a[0]^(aad_block)->a[0], (&tmp1)->a[1]=(&ctx->offset_aad)->a[1]^(aad_block)->a[1] );
ctx->encrypt(tmp1.c, tmp2.c, ctx->keyenc);
( (&ctx->sum)->a[0]=(&ctx->sum)->a[0]^(&tmp2)->a[0], (&ctx->sum)->a[1]=(&ctx->sum)->a[1]^(&tmp2)->a[1] );
}
last_len = len % 16;
if (last_len > 0) {
( (&ctx->offset_aad)->a[0]=(&ctx->offset_aad)->a[0]^(&ctx->l_star)->a[0], (&ctx->offset_aad)->a[1]=(&ctx->offset_aad)->a[1]^(&ctx->l_star)->a[1] );
memset(&tmp1, 0, 16);
memcpy(&tmp1, aad + (num_blocks * 16), last_len);
((unsigned char *)&tmp1)[last_len] = 0x80;
( (&tmp2)->a[0]=(&ctx->offset_aad)->a[0]^(&tmp1)->a[0], (&tmp2)->a[1]=(&ctx->offset_aad)->a[1]^(&tmp1)->a[1] );
ctx->encrypt(tmp2.c, tmp1.c, ctx->keyenc);
( (&ctx->sum)->a[0]=(&ctx->sum)->a[0]^(&tmp1)->a[0], (&ctx->sum)->a[1]=(&ctx->sum)->a[1]^(&tmp1)->a[1] );
}
...
if (lookup == ((void *)0))
return 0;
( (&ctx->offset)->a[0]=(&ctx->offset)->a[0]^(lookup)->a[0], (&ctx->offset)->a[1]=(&ctx->offset)->a[1]^(lookup)->a[1] );
inblock =
(OCB_BLOCK *)(in + ((i - ctx->blocks_processed - 1) * 16));
( (&tmp1)->a[0]=(&ctx->offset)->a[0]^(inblock)->a[0], (&tmp1)->a[1]=(&ctx->offset)->a[1]^(inblock)->a[1] );
( (&ctx->checksum)->a[0]=(&ctx->checksum)->a[0]^(inblock)->a[0], (&ctx->checksum)->a[1]=(&ctx->checksum)->a[1]^(inblock)->a[1] );
ctx->encrypt(tmp1.c, tmp2.c, ctx->keyenc);
outblock =
(OCB_BLOCK *)(out + ((i - ctx->blocks_processed - 1) * 16));
( (outblock)->a[0]=(&ctx->offset)->a[0]^(&tmp2)->a[0], (outblock)->a[1]=(&ctx->offset)->a[1]^(&tmp2)->a[1] );
}
}
last_len = len % 16;
if (last_len > 0) {
( (&ctx->offset)->a[0]=(&ctx->offset)->a[0]^(&ctx->l_star)->a[0], (&ctx->offset)->a[1]=(&ctx->offset)->a[1]^(&ctx->l_star)->a[1] );
...
((unsigned char *)(&tmp1))[last_len] = 0x80;
( (&ctx->checksum)->a[0]=(&ctx->checksum)->a[0]^(&tmp1)->a[0], (&ctx->checksum)->a[1]=(&ctx->checksum)->a[1]^(&tmp1)->a[1] );
}
...
(OCB_BLOCK *)(out + ((i - ctx->blocks_processed - 1) * 16));
( (outblock)->a[0]=(&ctx->offset)->a[0]^(&tmp2)->a[0], (outblock)->a[1]=(&ctx->offset)->a[1]^(&tmp2)->a[1] );
( (&ctx->checksum)->a[0]=(&ctx->checksum)->a[0]^(outblock)->a[0], (&ctx->checksum)->a[1]=(&ctx->checksum)->a[1]^(outblock)->a[1] );
}
}
last_len = len % 16;
if (last_len > 0) {
...
{
OCB_BLOCK tmp1, tmp2;
( (&tmp1)->a[0]=(&ctx->checksum)->a[0]^(&ctx->offset)->a[0], (&tmp1)->a[1]=(&ctx->checksum)->a[1]^(&ctx->offset)->a[1] );
( (&tmp2)->a[0]=(&tmp1)->a[0]^(&ctx->l_dollar)->a[0], (&tmp2)->a[1]=(&tmp1)->a[1]^(&ctx->l_dollar)->a[1] );
ctx->encrypt(tmp2.c, tmp1.c, ctx->keyenc);
( (&ctx->tag)->a[0]=(&tmp1)->a[0]^(&ctx->sum)->a[0], (&ctx->tag)->a[1]=(&tmp1)->a[1]^(&ctx->sum)->a[1] );
if (len > 16 || len < 1) {
return -1;
}
if (tag)
...
CRYPTO_ocb128_finish(ctx, ((void *)0), 0);
...
{
if (ctx) {
CRYPTO_clear_free(ctx->l, ctx->max_l_index * 16, "crypto\\modes\\ocb128.c", 563);
OPENSSL_cleanse(ctx, sizeof(*ctx));
}
}
#line 569 "crypto\\modes\\ocb128.c"
